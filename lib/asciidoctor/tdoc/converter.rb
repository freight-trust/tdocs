...
module Asciidoctor
  module tdoc
    # A {Converter} implementation that generates tdoc output, and a document
    # schema encapsulation of the document for validation
    #
    class Converter < Asciidoctor::Generic::Converter
      XML_ROOT_TAG = 'tdoc-standard'.freeze
      XML_NAMESPACE = 'https://www.metanorma.org/ns/tdoc'.freeze
      ...
      def html_converter(node)
        IsoDoc::tdoc::HtmlConvert.new(html_extract_attributes(node))
      end

      def pdf_converter(node)
        return nil if node.attr("no-pdf")
        IsoDoc::tdoc::PdfConvert.new(html_extract_attributes(node))
      end

      def word_converter(node)
        IsoDoc::tdoc::WordConvert.new(doc_extract_attributes(node))
      end
    end
  end
end
