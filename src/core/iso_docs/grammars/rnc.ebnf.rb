...
def ebnf_parse
  ...
  #
  char = /[\u0009\u000a\u000d\u0020-\u00ff]/.r
  name = /[:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff][:A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff.0-9\u00b7-]*/
    .r
  ...
  keyword =
    /attribute/.r | /default/.r | /datatypes/.r | /div/.r | /element/.r |
      /empty/.r | /externalRef/.r | /grammar/.r | /include/.r | /inherit/.r |
      /list/.r | /mixed/.r | /namespace/.r | /notAllowed/.r | /parent/.r |
      /start/.r | /string/.r | /text/.r | /token/.r
  quotedIdentifier = seq('\\', ncname)
  identifier = seq(''.r ^ keyword, ncname) | quotedIdentifier # identifierOrKeyword = identifier | keyword
...
=end
  ...
  decl =
    seq_(/namespace/.r, identifierOrKeyword, /=/.r, namespaceUri) |
      seq_(
        /default/.r,
        /namespace/.r,
  ...
  assignMethod = /=/.r | /\|=/.r | /\&=/.r
  start =
    seq_(/start/.r, assignMethod, lazy { pattern }).map { |t| t.join(' ') }
  define =
    seq_(identifier, assignMethod, lazy { pattern }).map do |id, eq, p|
      $defs[id] = [id, eq, p].join(' ')
      []
    end
  includeContent =
    define | start | seq_(/div/.r, /\{/.r, lazy { includeContent }.star, /\}/.r)
...
=end
  ...
  pattern1 =
    seq_(/[?*+]/.r._?, /[,&|]/.r, lazy { pattern }) * (1..-1) | /[?*+]/.r
  grammartail =
    seq_(/\}/.r, pattern1._?).map { |x, p| "INSERT_HERE\n}\n#{p[0]}" } |
      seq_(grammarContent, lazy { grammartail }).map { |t| t.join(' ') }
  pattern =
    seq_(/element/.r, nameClass, /\{/.r, lazy { pattern }, /\}/.r, pattern1._?)
      .map { |t| t.join(' ') + "\n" } |
      seq_(
        /attribute/.r,
        nameClass,
      ...
      seq_(/list|mixed/.r, /\{/.r, lazy { pattern }, /\}/.r, pattern1._?)
        .map { |t| t.join(' ') + "\n" } |
      seq_(
      ...
      seq_(datatypeName._?, datatypeValue, pattern1._?).map do |t|
        t.join(' ')
      end |
      seq_(
        datatypeName,
        seq_(/\{/.r, param.star, /\}/.r)._?,
        exceptPattern._?,
        pattern1._?
      ).map { |t| t.join(' ') } |
      seq_(/externalRef/.r, uri, inherit._?, pattern1._?).map do |t|
        t.join(' ') + "\n"
      end |
      seq_(/\(/.r, lazy { pattern }, /\)/.r, pattern1._?).map do |t|
        t.join(' ')
      end
  topLevel =
    seq_(decl.star, pattern, /\s*/.r).map do |d, t|
      $decl = d.join(' ')
      t #.join (' ') + "\n"
    end |
      seq_(decl.star, grammarContent * (1..-1), /\s*/.r).map do |d, t|
        $decl = d.join(' ')
        t.join (' ') + "\n"
      end
  fred =
...
def parse(filename)
  grammar =
    File.read(filename).gsub(/\s*$/m, '').lines.reject do |l|
      l =~ /^\s*#/
    end.join('')
  @ctx = Rsec::ParseContext.new grammar, 'source'
  ret = ebnf_parse._parse @ctx
  raise @ctx.generate_error 'source' if !ret or Rsec::INVALID[ret]
  ret
end

ret1 = parse(ARGV[0]) # incredibly, I can't detect start, so I have to extract it
