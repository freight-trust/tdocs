...
require 'rexml/document' # require 'diagrams-xml'
...
module EDI
  ...
  class Interchange
    ...
    def Interchange.parse_xml(hnd)
      ...
      EDI.const_get(key).const_get('Interchange').parse_xml(hnd) #      EDI::const_get(class_sym).parse_xml( hnd )
    end

    def to_xml(xel_parent)
      externalID = "PUBLIC\n" + ' ' * 9
      externalID +=
        "'-//FH Wiesbaden FB DCSM//DTD XML Representation of EDI data V1.2//EN'\n"
      externalID += ' ' * 9
      externalID +=
        "'http://edi01.informatik.fh-wiesbaden.de/edi4r/edi4r-1.2.dtd'"
  ...
  class MsgGroup
    ...
    def MsgGroup.parse_xml(p, xgrp)
      ...
      xgrp.elements.each('Message') do |xel|
        grp.add Message.parse_xml(grp, xel)
      end
