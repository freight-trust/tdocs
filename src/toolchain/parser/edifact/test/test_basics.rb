...
class EDIFACT_Tests < Test::Unit::TestCase
  ...
  def fill_ORDERS(msg)
    ...
    msg.add seg
    seg.d1004 = '220'
    assert_equal(%w[C002 1004 1225 4343], seg.names)
    ...
    assert_equal(%w[2005 2380 2379], seg.cC507.names)
    seg = msg.new_segment('UNS')
    seg.d0081 = 'S'
    msg.add seg #    p seg.names
  end
  ...
  def fill_INVOIC(msg)
    ...
    msg.add seg
    seg.cC002.d1001 = '380'
    assert_equal(%w[C002 C106 1225 4343], seg.names)
    seg = msg.new_segment('DTM')
    msg.add seg
    seg.cC507.d2005 = '137'
    t = Time.new
    ...
    msg.add seg
    seg.cC516.d5025 = '86'
    seg.cC516.d5004 = 0.0 #    p seg.names
  end
  ...
  def test_interchange_creation
    ...
    assert_match(
      /^UNA:\+\.\? 'UNB\+UNOB:3\+\+\+\d{6}:\d{4}\+1'UNZ\+0\+1'$/,
      ic.to_s
    )
    ...
    assert_match(
      /^UNB\+UNOC:3\+\+\+\d{6}:\d{4}\+(123)\+{6}1'UNZ\+0\+\1'$/,
      ic.to_s
    )

    # Now test special separator case SV2, UNOB. Is UNA on by default?
    assert_nothing_raised { ic = EDI::E::Interchange.new({ version: 2 }) }
    assert_match(
      /^UNA\021\022\.\? \024UNB\022UNOB\0212\022\022\022\d{6}\021\d{4}\0221\024UNZ\0220\0221\024$/,
      ic.to_s
    )
    ...
    assert_nothing_raised do
      ic =
        EDI::E::Interchange.new({ show_una: true, version: 2, charset: 'UNOB' })
    end
    assert_match(
      /^UNA\021\022\.\? \024UNB\022UNOB\0212\022\022\022\d{6}\021\d{4}\0221\024UNZ\0220\0221\024$/,
      ic.to_s
    ...
    # SV4 working?
    assert_nothing_raised do
      ic = EDI::E::Interchange.new({ version: 4, charset: 'UNOC' })
    end
    assert_match(
      /^UNA:\+\.\?\*'UNB\+UNOC:4\+\+\+\d{8}:\d{4}\+1'UNZ\+0\+1'$/,
      ic.to_s
    )
    ...
    end #    assert_match( /^UNA:\+\.\?\*'UIB\+UNOC:4\+\+\+\d{8}:\d{4}\+1'UIZ\+0\+1'$/, ic.to_s )
  end
  ...
  begin
    def test_interchange_parsing
      ...
      assert_nothing_raised do
        ...
        ic.output_mode = :indented
        File.open('remadv.out', 'w') { |hnd| hnd.print ic } #      $stdout.write ic
      end
    end
  end
end
